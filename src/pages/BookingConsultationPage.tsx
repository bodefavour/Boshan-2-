...
const BookConsultationPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F5] text-black py-10 px-6 max-w-3xl mx-auto">
      <motion.h1
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-xl shadow-md">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} required placeholder="Phone Number" className="input" />
          <select value={skinType} onChange={(e) => setSkinType(e.target.value)} required className="input">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {skinConcernOptions.map((c) => (
              <label key={c} className="flex items-center gap-2 text-sm">
                <input type="checkbox" value={c} checked={concerns.includes(c)} onChange={() => toggleConcern(c)} />
                {c}
              </label>
            ))}
          </div>
        ...
        <button type="submit" className="w-full bg-orange-600 hover:bg-orange-700 text-white py-3 rounded-full font-medium">
          Confirm Booking
        </button>
      </form>
    </div>
